...
_.extend(Mongo.prototype, {
  ...
  addEvents: function (events, callback) {
    ...
    if (events.length === 1) {
      this.events.insert(events, function(err) {
        if (err) {
          debug(err);
        }
        if (callback) callback(err);
  ...
  repairFailedTx: function (lastEvt, callback) {
    var self = this;
    this.transactions.findOne({_id: lastEvt.commitId}, function (err, tx) {
      ...
      if (!tx) {
        var err = new Error('missing tx entry with _id ' + lastEvt.commitId);
        debug(err);
        callback(err);
        return;
      }
  ...
  getEventsByRevision: function (query, revMin, revMax, callback) {
    ...
    this.events.find(findStatement, { sort: [['streamRevision', 'asc']] }).toArray(function (err, res) {
      ...
      if (lastEvt.restInCommitStream === 0 || !lastEvt.restInCommitStream) {
        ...
        self.repairFailedTx(lastEvt, function(err) {
          if (err) {
            debug(err);
            callback(err);
            return;
  ...
  getSnapshot: function (query, revMax, callback) {
    ...
    this.snapshots.findOne(findStatement, { sort: [['revision', 'desc']] }, callback);
  }
